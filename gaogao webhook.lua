...
local function saveConfig()
    ...
    pcall(function()
        writefile(CONFIG_FILE, HttpService:JSONEncode({
            ...
            webhookMerchantEnabled = webhookMerchantEnabled
        }))
    end)
end
    ...
    pcall(function()
        ...
        if data then
            ...
            if type(config) == "table" then
                ...
                webhookMerchantEnabled = config.webhookMerchantEnabled == true
            end
        end
    end)
end
...
local function inspectMerchantModel(candidate)
    ...
    for _, attr in ipairs({ "MerchantName", "merchant_name", "MerchantID", "merchantId" }) do
        local v = model:GetAttribute(attr)
        if v and type(v) == "string" and v ~= "" then
            local n = trimText(v)
            if n ~= "" and not looksLikeGuid(n) then
                merchantName = n
                break
            end
        end
    ...
    if not merchantName then
        ...
        if prompt then
            merchantName = trimText(prompt.ObjectText)
        end
    end

    if not merchantName or merchantName == "" then return nil, nil end
...
local function sendMerchantWebhook(merchantName, merchantModel)
    ...
    local detectedAt = discordTimestamp()
    local payload = makeBonesEmbed(merchantName .. " has arrived on the island.", privateServerUrl, detectedAt, 16763904)
    httpPost(webhookUrl, HttpService:JSONEncode(payload))
end
...
local function startMerchantScan()
    ...
    webhookMerchantAddedConn = workspace.DescendantAdded:Connect(function(obj)
        if obj:IsA("Model") or obj:IsA("Humanoid") or obj:IsA("ProximityPrompt") then
            ...
            task.delay(2.5, inspectAndSendMerchant, obj)
        end
    end)
    ...
    webhookMerchantThread = task.spawn(function()
        while _G.BonesHubWebhookRunToken == WEBHOOK_RUN_TOKEN
            ...
            task.wait(4)
            if webhookMerchantRunning then scanExistingMerchants() end
        end
    end)
end
